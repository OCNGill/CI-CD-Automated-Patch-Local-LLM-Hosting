...
If you intentionally need remote access, follow `SECURITY.md` and run the UI behind an authenticated reverse proxy (TLS + auth). Do not change `--server.address` to `0.0.0.0` on an untrusted network.
