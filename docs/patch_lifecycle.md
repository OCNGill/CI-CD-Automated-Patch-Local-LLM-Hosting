...
# Security note: Atlas defaults to requiring manual confirmation before apply and push operations.
# See `atlas_core/config/llm_config.yaml` for `safety.enable_auto_apply`, `safety.enable_master_push`, and
# `safety.require_manual_confirmation` defaults (recommended: false, false, true).
```
