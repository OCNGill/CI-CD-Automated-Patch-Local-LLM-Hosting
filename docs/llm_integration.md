...
## Local-first & Security (Important)

Atlas is intentionally designed to be local-first. The following operational security guidance is mandatory for safe use:

- Default bindings: Atlas ships with local-only defaults. LLM endpoints and the UI are expected to bind to `127.0.0.1` by default. Do not change to `0.0.0.0` unless you understand the implications.
- Per-machine configuration: Each operator must configure their own `atlas_core/config/llm_config.yaml`. Do not share or commit machine-specific secrets or tokens.
- When enabling remote access:
  - Use a reverse proxy with TLS and authentication (OAuth2, Basic+ACLs, or mTLS).
  - Limit network exposure with firewall rules and VPNs.
- GitHub integration: Store push tokens (e.g., `ATLAS_PUSH_TOKEN`) in your GitHub secrets. Use least-privilege tokens and require typed manual confirmations before any pushes.
- Auditing: Atlas maintains append-only provenance logs (JSONL). Review them for any unexpected activity.

If you need help converting a local-only setup to a secure remote setup, follow the steps in `SECURITY.md`.
